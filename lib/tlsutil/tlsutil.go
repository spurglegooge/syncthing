...
var ErrIdentificationFailed = errors.New("failed to identify socket type")

// The list of cipher suites we will use / suggest for TLS 1.2 connections is
// defined per build: ciphers_default.go for standard builds and
// ciphers_fips.go for FIPS builds (which omit the non-approved ChaCha20
// suites). These are ignored for TLS 1.3.
